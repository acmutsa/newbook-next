...
import { clearCourses, insertCourses } from "db/queries";
import { type CourseScrapeData } from "db/types";
import { db } from "db";
...
const main = async () => {
	...
	for (const option of optionList) {
		...
		await optionInput?.evaluate(
			(el, option) => (el.value = option),
			option,
		);
		...
		while (true) {
			...
			const courseList: CourseScrapeData[] = await Promise.all(
